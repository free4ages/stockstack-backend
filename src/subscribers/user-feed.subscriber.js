...
const sendToFeedOnTagAdd = async (payload) => {
  ...
  const [article,tags] = await Promise.all([
    articleService.getArticleInstance(articleId,{raise:true}),
    fetchTags,
  ]);
  console.log(tags);
  ...
  const addArticleinFeedTasks = async ()=>{
    ...
    const subscribedUserIds = (await UserTag.find({tag:{$in:selectedTagIds}},{user:1})).map(userTag=> userTag.user);
    console.log("Subscribed UserIds",subscribedUserIds);
    //get all users in userIds who dont have article in feed
    const userIdswithArticle = (await UserFeed.find({user:{$in:subscribedUserIds},article:article._id},{user:1})).map(userFeed => userFeed.user);
    console.log("User Ids with article",userIdswithArticle)

    const userIdswithoutArticle = _.differenceWith(subscribedUserIds,userIdswithArticle,(a,b) => a.toString()===b.toString());
